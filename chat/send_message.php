...
try {
    ...
    if ($result['success']) {
        ...
        // ── Bot 自动回复 ──
        if ($chat_type === 'friend' && !empty($message_text)) {
            botAutoReply($conn, $message, $user_id, $friend_id, $message_text);
        }
    } else {
...
// ========== Bot 自动回复函数 ==========
function botAutoReply(PDO $conn, Message $message, int $userId, int $friendId, string $text): void {
    // 检查对方是否是 Bot
    $BOT_EMAIL = 'bot@mummories.local';
    $stmt = $conn->prepare("SELECT id FROM users WHERE id = ? AND email = ?");
    $stmt->execute([$friendId, $BOT_EMAIL]);
    if (!$stmt->fetch()) return; // 不是 Bot，直接返回
    ...
    $rules = [
        // 问候
        ['keys' => ['你好', 'hi', 'hello', '嗨', '在吗', '在不在'], 'reply' => '你好呀！我是 Mummories 助手 🤖 有什么可以帮你的吗？'],
        // 功能介绍
        ['keys' => ['功能', '你能做什么', '你会什么', '帮助', 'help'], 'reply' => "我可以帮你：\n📝 了解网站功能\n👤 修改个人资料\n🔒 隐私设置\n💬 聊天使用技巧\n🎨 主题和背景设置\n\n直接输入关键词就能获取帮助哦~"],
        // 头像
        ['keys' => ['头像', '换头像', '上传头像'], 'reply' => '修改头像：点击左侧个人资料区的头像，选择图片即可上传新头像 📷'],
        // 隐私
        ['keys' => ['隐私', '隐私设置', '保护'], 'reply' => '隐私设置在左侧侧栏的「隐私设置」入口，你可以控制谁能看到你的主页、技能、邮箱等信息 🔒'],
        // 技能
        ['keys' => ['技能', '标签', 'skill'], 'reply' => '编辑技能：在首页技能板块点击编辑按钮（✏️），可以添加、删除技能标签 🏷️'],
        // 文章
        ['keys' => ['文章', '写文章', '发布文章'], 'reply' => '发布文章：进入「文章」页面，点击右上角的发布按钮即可创建新文章 ✍️'],
        // 主题/背景
        ['keys' => ['主题', '背景', '皮肤', '壁纸'], 'reply' => '自定义外观：点击右上角设置图标，可以更换背景图片和主题颜色 🎨'],
        // 说说/动态
        ['keys' => ['说说', '动态', '心情'], 'reply' => '发说说：进入「说说」页面，在顶部输入框写下你的心情，支持图片和表情哦 💭'],
        // 群聊
        ['keys' => ['群聊', '建群', '创建群'], 'reply' => '创建群聊：在聊天室界面点击右上角的 + 按钮，选择「创建群聊」，邀请好友一起聊天 👥'],
        // 密码
        ['keys' => ['密码', '修改密码', '忘记密码'], 'reply' => '修改密码：进入「隐私设置」→「修改密码」标签页，输入旧密码和新密码即可 🔑'],
        // 好友
        ['keys' => ['加好友', '添加好友', '搜索好友'], 'reply' => '添加好友：在聊天室界面点击搜索图标，输入对方用户名或邮箱搜索并发送好友请求 🤝'],
        // 表情/趣味
        ['keys' => ['谢谢', '感谢', 'thanks'], 'reply' => '不客气～有问题随时找我！😊'],
        ['keys' => ['哈哈', '笑', '有趣'], 'reply' => '开心就好～ 😄'],
        ['keys' => ['再见', 'bye', '拜拜'], 'reply' => '再见～下次见！👋'],
    ];
    ...
    if ($reply === null) {
        $defaults = [
            '我还在学习中...试试输入「帮助」看看我能做什么 🤔',
            '这个问题我暂时回答不了，输入「功能」查看我能帮你的事情 📋',
            '嗯...我没太理解你的意思。你可以试试问我：头像、隐私设置、技能、主题 等问题哦 💡',
            '我是 Mummories 助手，还在成长中！试试关键词：帮助、头像、密码、群聊 🌱',
        ];
        $reply = $defaults[array_rand($defaults)];
    }

    // 延迟模拟"正在输入"效果（0.5-1.5秒随机）
    usleep(random_int(500000, 1500000));

    // 发送 Bot 回复
    $message->sendTextMessage($botId, $userId, $reply);
}