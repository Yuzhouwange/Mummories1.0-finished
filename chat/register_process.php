...
$email_verify = false;
    ...
    if ($result['success']) {
        ...
        // 自动添加 Bot 为好友并发送欢迎消息
        $botStmt = $conn->prepare("SELECT id FROM users WHERE email = 'bot@mummories.local' AND is_deleted = FALSE LIMIT 1");
        $botStmt->execute();
        $botRow = $botStmt->fetch();
        if ($botRow) {
            $botId = (int)$botRow['id'];
            $newUid = (int)$result['user_id'];
            try {
                $conn->prepare("INSERT IGNORE INTO friends (user_id, friend_id, status) VALUES (?, ?, 'accepted')")->execute([$botId, $newUid]);
                $conn->prepare("INSERT IGNORE INTO friends (user_id, friend_id, status) VALUES (?, ?, 'accepted')")->execute([$newUid, $botId]);
                require_once 'Message.php';
                $msgObj = new Message($conn);
                $msgObj->sendTextMessage($botId, $newUid, '欢迎加入 Mummories！我是你的助手 🤖 输入「帮助」查看我能做的事情~');
            } catch (PDOException $e) {
                error_log("自动添加Bot好友失败: " . $e->getMessage());
            }
        }
