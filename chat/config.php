...
if (session_status() === PHP_SESSION_NONE) {
    // 校验 PHPSESSID 格式，防止外部传入非法 session ID 导致 500
    if (isset($_COOKIE[session_name()]) && !preg_match('/^[a-zA-Z0-9,-]+$/', $_COOKIE[session_name()])) {
        unset($_COOKIE[session_name()]);
        setcookie(session_name(), '', time() - 3600, '/chat/');
    }
    // 设置 session cookie 路径为 /chat/，避免与 Mummories 主站 session 冲突
...
define('DB_HOST', getEnvVar('DB_HOST') ?: getEnvVar('DB_HOSTNAME') ?: 'db');
