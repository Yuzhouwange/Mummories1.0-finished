...
function handleBlog(mysqli $db, string $method, array $body, string $ip, array $segs): void {
    ...
    if ($action === 'register' && $method === 'POST') {
        ...
        // 自动添加 Bot 助手为好友并发送欢迎消息，并写入 chat session
        try {
            global $env;
            $chatDb = @new mysqli(
                $env['DB_HOST'] ?? 'db',
            ...
            if (!$chatDb->connect_error) {
                ...
                error_log("BLOG_DEBUG: 注册/写入 chat session user_id=$chatUserId");
                                file_put_contents(__DIR__ . '/test_session.txt', 'register user_id=' . $_SESSION['user_id'] . "\n", FILE_APPEND);
                session_write_close();
                session_name('HP_ADMIN_SID');
                session_id($hpSid);
                session_start();
    ...
    elseif ($action === 'login' && $method === 'POST') {
        ...
        try {
            ...
            if (!$chatDb->connect_error) {
                $chatDb->set_charset('utf8mb4');
                $emailC = $chatDb->real_escape_string($user['email']);
                $cr = $chatDb->query("SELECT id FROM users WHERE email='$emailC' LIMIT 1");
                if ($cr && $row = $cr->fetch_assoc()) $chatUserId = (int)$row['id'];
                // 强制写入 chat session
                $hpSid = session_id();
                ...
                $_SESSION['avatar']   = !empty($user['avatar']) ? '/avatars/' . $user['avatar'] : '';
                $_SESSION['last_activity'] = time();
                error_log("BLOG_DEBUG: 登录/写入 chat session user_id=$chatUserId");
                                file_put_contents(__DIR__ . '/test_session.txt', 'login user_id=' . $_SESSION['user_id'] . "\n", FILE_APPEND);
                session_write_close();
                session_name('HP_ADMIN_SID');
                session_id($hpSid);
                session_start();
            ...
            error_log("登录时同步 chat session 失败: " . $e->getMessage());
        }
...
function handleUser(mysqli $db, string $method, array $body, ?int $blogUserId, ?array $blogUser, array $segs = []): void {
    ...
    elseif ($method === 'POST') {
        ...
        // 同步到 chat.users 表（跨库更新，匹配 email），并强制写入 chat session
        try {
            global $env;
            $chatDb = @new mysqli(
                $env['DB_HOST'] ?? 'db',
            ...
            if (!$chatDb->connect_error) {
                ...
                $avatarUrlE = $chatDb->real_escape_string($avatarUrl);
                $chatDb->query("UPDATE users SET avatar='$avatarUrlE' WHERE email='$emailE'");
                $cr = $chatDb->query("SELECT id FROM users WHERE email='$emailE' LIMIT 1");
                if ($cr && $row = $cr->fetch_assoc()) $chatUserId = (int)$row['id'];
                // 强制写入 chat session
                $hpSid = session_id();
                session_write_close();
                ...
                $_SESSION['email']    = $blogUser['email'];
                $_SESSION['avatar']   = $avatarUrl;
                $_SESSION['last_activity'] = time();
                error_log("BLOG_DEBUG: 头像/写入 chat session user_id=$chatUserId");
                                file_put_contents(__DIR__ . '/test_session.txt', 'avatar user_id=' . $_SESSION['user_id'] . "\n", FILE_APPEND);
                session_write_close();
                session_name('HP_ADMIN_SID');
                session_id($hpSid);
                session_start();
            ...
            error_log("头像上传时同步 chat session 失败: " . $e->getMessage());
        }
